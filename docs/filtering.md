...
| `--collapse <pattern>` | Replace matching modules with a single empty `package "…" { }` block; redirect edges that used to terminate inside them to the package. |

Precedence, applied per node: `exclude` wins over `include`, and
`collapse` applies only to nodes that survived both.
